...
class Timestore
{
    ...
    */
    ...
    {
        require "Modules/feedspectrum/engine/TimestoreApi.php";
    ...
    {
        $newfeedspectruminterval = (int) $options['interval'];
        if ($newfeedspectruminterval<5) $newfeedspectruminterval = 5;
        $this->timestoreApi->create_node($feedspectrumid,$newfeedspectruminterval);

        if (file_exists($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT).".tsdb")) return true;
        return false;
    }
    ...
    public function post($feedspectrumid,$time,$value)
    {
        ...
        {
            $this->timestoreApi->post_values($feedspectrumid,$time*1000,array($value),null);
        }
    ...
    public function get_data($feedspectrumid,$start,$end,$outinterval)
    {
        ...
        $meta = $this->get_meta($feedspectrumid);
        ...
        $data = json_decode($this->timestoreApi->get_series($feedspectrumid,0,$npoints,$start,$end,null));
        return $data;
    }
    ...
    public function get_average($feedspectrumid,$start,$end,$interval)
    {
        $feedspectrumid = intval($feedspectrumid);
        $start = intval($start/1000);
        $end = intval($end/1000);
    ...
    public function scale_range($feedspectrumid,$start,$end,$value)
    {
        ...
        {
            $time = $point[0];
            $this->timestoreApi->post_values($feedspectrumid,$time,array($point[1] * $value),null);
        }

        return true;
    }

    public function export($feedspectrumid,$start,$layer)
    {
        $feedspectrumid = (int) $feedspectrumid;
        $start = (int) $start;
        $layer = (int) $layer;
        ...
        $primary = fopen($this->dir.$feedspectrumname, 'rb');
        $primarysize = filesize($this->dir.$feedspectrumname);
    ...
    {
        $this->timestoreApi->delete_node($feedspectrumid);
    }
