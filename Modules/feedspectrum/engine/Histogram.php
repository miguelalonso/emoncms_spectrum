...
class Histogram
{
    ...
    */
     ...
     *
     * @param integer $feedspectrumid The feedspectrumid of the histogram table
     * @param integer $start  Start time UNIX Timestamp
     * @param integer $end    Start time UNIX Timestamp
     *
     * @return array of power vs kwh
    */
    public function get_power_vs_kwh($feedspectrumid,$start,$end)
    {
        ...
        $result = $this->mysqli->query("select data2, sum(data) as kWh from $feedspectrumname WHERE time>='$start' AND time<'$end' group by data2 order by data2 Asc");
    ...
    */
    public function get_kwhd_atpower($feedspectrumid, $min, $max)
    {
        $feedspectrumid = intval($feedspectrumid);
        $min = intval($min);
        $max = intval($max);
        ...
        $feedspectrumname = "feedspectrum_".trim($feedspectrumid)."";
        $result = $this->mysqli->query("SELECT time, sum(data) as kWh FROM `$feedspectrumname` WHERE `data2`>='$min' AND `data2`<='$max' group by time");
    ...
    */
    public function get_kwhd_atpowers($feedspectrumid, $points)
    {
        $feedspectrumid = intval($feedspectrumid);
        $feedspectrumname = "feedspectrum_".trim($feedspectrumid)."";
