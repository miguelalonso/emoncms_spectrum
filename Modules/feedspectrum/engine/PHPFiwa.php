...
class PHPFiwa
{
    ...
    */
    public function create($id,$options)
    {
        ...
        {
            $this->log->info("PHPFIWA:create creating feedspectrum id=$id");
            // Set initial feedspectrum meta data
            $meta = new stdClass();
            $meta->id = $id;
            $meta->start_time = 0;
            
            // Limitation's on feedspectrum interval's so that the next layer can always be produced from an
        ...
        $feedspectrumname = "$id.meta";
        if (file_exists($this->dir.$feedspectrumname)) return true;
        return false;
    }
    ...
    */
    ...
    {   
        ...
        // If this is a new feedspectrum (npoints == 0) then set the start time to the current datapoint
        ...
        if ($timestamp < $meta->start_time) {
            $this->log->warn("PHPFiwa:post timestamp older than feedspectrum start time id=$id");
    ...
    private function update_layer($meta,$layer,$point,$timestamp,$value)
    {
        ...
        if (!flock($fh, LOCK_EX)) {
            $this->log->warn("PHPFiwa:update_layer data file for layer=$layer feedspectrumid=".$meta->id." is locked by another process");
            fclose($fh);
            return false;
        }
    ...
    */
    public function get_data_basic($feedspectrumid,$start,$end,$dp)
    {
    ...
    */
    ...
    public function export($id,$start,$layer)
    {
        ...
        $feedspectrumname = $id."_$layer.dat";
    ...
    public function get_feedspectrum_size($id)
    {
        if (!$meta = $this->get_meta($id)) return false;
    ...
    public function get_meta($id)
    {
        ...
        // print $this->dir.$feedspectrumname;
        ...
        if ($meta->nlayers<1 || $meta->nlayers>4) {
            $this->log->warn("PHPFiwa:get_meta feedspectrum:$id nlayers out of range");
            return false;
        }
        ...
        if ($meta->start_time <= 0 && $meta->npoints[0]>1) {
            $this->log->warn("PHPFiwa:get_meta feedspectrum:$id start time must be greater than zero");
        }
    ...
    public function recompile($meta)
    {
        ...
        // which mean the difference of days of calculation time on large systems with thousands of feedspectrums.
