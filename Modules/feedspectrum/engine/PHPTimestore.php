...
class PHPTimestore
{
    ...
    public function create($feedspectrumid,$options)
    {
        ...
        // Check to ensure we dont overwrite an existing feedspectrum
        if (!$meta = $this->get_meta($feedspectrumid)) {

            // Set initial feedspectrum meta data
            $meta = new stdClass();
            $meta->feedspectrumid = $feedspectrumid;
            $meta->npoints = 0;
            $meta->start = 0;
            $meta->nmetrics = 1;
            ...
            // Save meta data
            $this->create_meta($feedspectrumid,$meta);
            
            for ($l=0; $l<6; $l++) {
                $fh = fopen($this->dir.str_pad($meta->feedspectrumid, 16, '0', STR_PAD_LEFT)."_".$l."_.dat", 'c+');
                if (!$fh) {
                    $this->log->warn("PHPTimestore:create could not create data file for layer $l feedspectrumid=$feedspectrumid");
                    return false;
                } else {
                    fclose($fh);
                }
        ...
        if (file_exists($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT).".tsdb")) {
            return true;
        } else {
            $this->log->warn("PHPTimestore:create meta file does not exist id=$feedspectrumid");
            return false;
        }
    }

    public function post($feedspectrumid, $timestamp, $value)
    {
        $this->log->info("PHPTimestore:post id=$feedspectrumid timestamp=$timestamp value=$value");
        ...
        if (!$meta = $this->get_meta($feedspectrumid)) {
            $this->log->warn("PHPTimestore:post failed to fetch meta id=$feedspectrumid");
            return false;
        }
        ...
        if ($timestamp < $meta->start) {
            $this->log->warn("PHPTimestore:post timestamp older than start time feedspectrumid=$feedspectrumid");
    ...
    {
      $this->post($feedspectrumid,$time,$value);
    }
    ...
    public function update_layer($meta,$layer,$point,$npoints,$value)
    {
        ...
        $fh = fopen($this->dir.str_pad($meta->feedspectrumid, 16, '0', STR_PAD_LEFT)."_".$layer."_.dat", 'c+');
        
        if (!$fh)
        {
            $this->log->warn("PHPTimestore:update_layer could not open data file for layer $layer feedspectrumid=$feedspectrumid");
            return false;
        }
        
        if (!flock($fh, LOCK_EX)) {
            $this->log->warn("PHPTimestore:update_layer data file for layer=$layer feedspectrumid=$feedspectrumid is locked by another process");
            fclose($fh);
            return false;
        }
        ...
        if ($point > $npoints) {
            ...
            if ($npadding>2500000) {
                $this->log->warn("PHPTimestore:update_layer npadding=$npadding > 2500000! exit feedspectrumid=$feedspectrumid");
                return false;
            }
    ...
    public function get_data($feedspectrumid,$start,$end,$outinterval)
    {
        ...
        if (!$meta = $this->get_meta($feedspectrumid)) return false;
    ...
    public function tsdb_get_series($meta,$start,$end,$npoints)
    {
        ...
        /* Generate output points by averaging all available spectrum points between the start
         * and end times for each output step.  Output timestamps are rounded down onto the
         * spectrum interval - there is no interpolation. */
        ...
        $feedspectrumname = str_pad($meta->feedspectrumid, 16, '0', STR_PAD_LEFT)."_".$layer."_.dat";
        $primaryfeedspectrumname = $this->dir.$feedspectrumname;
        $fh = fopen($primaryfeedspectrumname, 'rb');
    ...
    public function get_data_alternative($feedspectrumid,$start,$end)
    {
        $decimation = array(20, 6, 6, 4, 7);
        //$feedspectrumid = 21; $start = 1317572497; $end = 1317692497; $dp = 10;
        $feedspectrumid = intval($feedspectrumid);
        $start = intval($start/1000);
        $end = intval($end/1000);
        $dp = 1000;
        ...
        $feedspectrumname = str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_".$layer."_.dat";
        $primaryfeedspectrumname = $this->dir.$feedspectrumname;
        $filesize = filesize($primaryfeedspectrumname);
    ...
    public function lastvalue($feedspectrumid)
    {
        ...
        $feedspectrumname = str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_0_.dat";

        $primaryfeedspectrumname = $this->dir.$feedspectrumname;

        if (file_exists($primaryfeedspectrumname))
        {
            $fh = fopen($primaryfeedspectrumname, 'rb');
            $size = filesize($primaryfeedspectrumname);
    ...
    public function scale_range($feedspectrumid,$start,$end,$value)
    {
        // not yet implemented
    }

    public function delete_range($feedspectrumid,$start,$end)
    {
        // not yet implemented
    }
    ...
    {
        $feedspectrumid = (int) $feedspectrumid;
        $feedspectrumname = str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT).".tsdb";

        if (!file_exists($this->dir.$feedspectrumname)) {
            //$this->log->warn("PHPTimestore:get_meta feedspectrum:$feedspectrumid metadata does not exist");
            return false;
        }
        $meta = new stdClass();
        $meta->feedspectrumid = $feedspectrumid;
        
        $size = filesize($this->dir.$feedspectrumname);
        
        if (!($size==36 || $size == 272)) {
            $this->log->warn("PHPTimestore:get_meta feedspectrum:$feedspectrumid metadata filesize error, size = $size");
            return false;
        } 
        
        $metafile = fopen($this->dir.$feedspectrumname, 'rb');

        fseek($metafile,8);
        $d = fread($metafile,8);
        $tmp = unpack("h*",$d);
        //$meta->feedspectrumid = (int) strrev($tmp[1]);
        ...
        if ($meta->nmetrics!=1) {
            $this->log->warn("PHPTimestore:get_meta feedspectrum:$feedspectrumid nmetrics is not 1");
            return false;
        }
        ...
        {
            $this->log->warn("PHPTimestore:get_meta feedspectrum:$feedspectrumid interval is out of range, interval is: ".$meta->interval);
            return false;
        }
        
        // Double verification of npoints
        
        clearstatcache($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_0_.dat");
        $filesize = filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_0_.dat");
        $meta->npoints = floor($filesize / 4.0);
        
        if ($meta->start <= 0 && $meta->npoints==1) {
          $this->log->warn("PHPTimestore:get_meta feedspectrum:$feedspectrumid start time must be greater than zero");
          return false;
        }
    ...
    public function create_meta($feedspectrumid,$meta)
    {
        $feedspectrumid = (int) $feedspectrumid;
        $feedspectrumname = str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT).".tsdb";

        $metafile = fopen($this->dir.$feedspectrumname, 'wb');
        
        if (!$metafile) {
            $this->log->warn("PHPTimestore:create_meta could not open metafile feedspectrumid=$feedspectrumid");
            return false;
        }
        
        if (!flock($metafile, LOCK_EX)) {
            $this->log->warn("PHPTimestore:create_meta ".$this->dir.$feedspectrumname." is locked by another process");
            fclose($metafile);
            return false;
        }
        ...
        fwrite($metafile,pack("I",0));
        fwrite($metafile,pack("h*",strrev(str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT))));
    ...
    {
        $size = 272;
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_0_.dat");
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_1_.dat");
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_2_.dat");
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_3_.dat");
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_4_.dat");
        $size += filesize($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_5_.dat");
        return $size;
    }

    public function export($feedspectrumid,$layer,$start)
    {
        $feedspectrumid = (int) $feedspectrumid;
        $layer = (int) $layer;
        $start = (int) $start;
    ...
    public function export_meta($feedspectrumid)
    {
        $feedspectrumid = (int) $feedspectrumid;
        ...
        $fh = @fopen( 'php://output', 'w' );
        $meta = fopen($this->dir.$feedspectrumname, 'rb');
    ...
    {
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT).".tsdb");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_0_.dat");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_1_.dat");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_2_.dat");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_3_.dat");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_4_.dat");
        unlink($this->dir.str_pad($feedspectrumid, 16, '0', STR_PAD_LEFT)."_5_.dat");
    }
    ...
    {
        $start += $out_interval;
            // Determine if this point is in-range of the spectrum table
            if ($start < $meta->start || $start >= $meta->start + $meta->npoints * $meta->interval) {
                    // No - there is no data at this time point
                    continue;
            }

            // There may be data for this point in the table.  Calculate the range of spectrum points
