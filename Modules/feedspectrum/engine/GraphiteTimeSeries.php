...
class GraphiteTimeSeries
{
    ...
    */
    ...
    /**
     * Utility method to convert a feedspectrumid to a Graphite target name
     *
     * @param integer $feedspectrumid The feedspectrumid of the timestore
     * @returns A string name of the Graphite target to use
    */
    private function get_graphite_graphname($feedspectrumid)
    {
        $feedspectrumid = intval($feedspectrumid);
        return "$this->root.feedspectrum_$feedspectrumid";
    }
    ...
    */
     ...
     *
     * @param integer $feedspectrumid The feedspectrumid of the timestore to fetch from
    ...
    */
    private function get_graphite_data($feedspectrumid,$start,$end)
    {
        $data = array();

        $feedspectrumname = $this->get_graphite_graphname($feedspectrumid);

        $url = "http://$this->host/render?target=$feedspectrumname&from=$start&until=$end&format=json";
        #error_log($url);
        $reader = fopen($url, "r");
    ...
    */
     ...
     *
     * @param integer $feedspectrumid The feedspectrumid of the timestore to be created
    */
     ...
     *
     * @param integer $feedspectrumid The feedspectrumid of the timestore to add to
    ...
    */
    ...
    {
        $feedspectrumname = $this->get_graphite_graphname($feedspectrumid);
        $line = "$feedspectrumname $value $time\n";
    ...
    */
    public function get_data($feedspectrumid,$start,$end,$dp)
    {
        ...
        $raw_datapoints = $this->get_graphite_data($feedspectrumid, $start, $end);
    ...
    */
    ...
    public function export($feedspectrumid,$start)
    {
        ...
        $feedspectrumname = get_graphite_graphname($feedspectrumid);
        $fileName = "feedspectrum_".trim(feedspectrumid).'.csv';
        ...
        $reader = @fopen("http://$this->host/render?target=$feedspectrumname&from=$start&format=raw", "r");
    ...
    {
        $this->post($feedspectrumid, $time, null);
    }
    ...
    public function deletedatarange($feedspectrumid,$start,$end)
    {
        ...
        $sum = 0;
        foreach ($raw_datapoints as $item) {
            $this->delete_data_point($feedspectrumid, $item[0]);
        }
        return true;
    }
