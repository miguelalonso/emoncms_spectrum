...
if (!$dashboardspectrum['height']) $dashboardspectrum['height'] = 400;
?>
...
<div id="widget_options" class="modal hide keyboard" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-header">
        ...
        <h3>Configurar cada elemento del dashboard</h3>
    </div>
...
<div style="background-color:#ddd; padding:4px;">
    ...
    <button id="save-dashboardspectrum" class="btn btn-success" style="float:right"><?php echo _('Not modified'); ?></button>
</div>
...
<div id="page-container" style="height:<?php echo $dashboardspectrum['height']; ?>px; position:relative;">
    ...
    <canvas id="can" width="940px" height="<?php echo $dashboardspectrum['height']; ?>px" style="position:absolute; top:0px; left:0px; margin:0; padding:0;"></canvas>
</div>


<script type="text/javascript" src="<?php echo $path; ?>Modules/dashboardspectrum/Views/js/designer.js"></script>
<script type="application/javascript">
    var dashid = <?php echo $dashboardspectrum['id']; ?>;
    var path = "<?php echo $path; ?>";
    var apikey = "";
    var feedspectrumlist = feedspectrum.list();
   ...
   $("#testo").hide();
    ...
    $("#save-dashboardspectrum").click(function (){
        ...
        //alert("&id="+dashid+'&content='+encodeURIComponent($("#page").html())+'&height='+designer.page_height);
        designer.page_height = 0;
        designer.scan();
        $.ajax({
            type: "POST",
            url :  path+"dashboardspectrum/setcontent.json",
            data : "&id="+dashid+'&content='+encodeURIComponent($("#page").html())+'&height='+designer.page_height,
            dataType: 'json',
            success : function(data) { console.log(data); if (data.success==true) $("#save-dashboardspectrum").attr('class','btn btn-success').text('<?php echo _("Saved") ?>');
            }
        });
    });
