/*
 ...
 The dashboardspectrum designer works around the concept of html elements with fixed positions
 ...
 the feedspectrumid to use is also specified in the html element:

 <div class="dial" feedspectrumid="1" style="position:absolute; top:50px; left:50px; width:200px; height:100px;" ></div>
 ...
 The dashboardspectrum designer creates a canvas layer above the dashboardspectrum html elements layer and uses jquery to get the mouse positions and actions that specify the box position and dimentions.
...
var designer = {
    ...
    'modified': function() {
        $("#save-dashboardspectrum").attr('class','btn btn-warning').text(_Tr("Changed, press to save"));
    },
    ...
    'draw': function()
    {
        designer.page_width = parseInt($('#dashboardspectrumpage').width());
        $('#can').width($('#dashboardspectrumpage').width());
    ...
    'draw_options': function(widget)
    {
        ...
        for (z in box_options)
        {
            ...
            if (options_type && options_type[z] == "feedspectrum")
            {
                options_html += "<td><select id='"+box_options[z]+"' class='options' >";
                for (i in feedspectrumlist)
                {
                    var selected = "";
                    if (val == feedspectrumlist[i]['name'].replace(/\s/g, '-'))
                        selected = "selected";
                    options_html += "<option value='"+feedspectrumlist[i]['name'].replace(/\s/g, '-')+"' "+selected+" >"+feedspectrumlist[i]['name']+"</option>";
                }
            }

            else if (options_type && options_type[z] == "feedspectrumid")
            {
                options_html += "<td><select id='"+box_options[z]+"' class='options' >";
                for (i in feedspectrumlist)
                {
                    var selected = "";
                    if (val == feedspectrumlist[i]['id'])
                        selected = "selected";
                    options_html += "<option value='"+feedspectrumlist[i]['id']+"' "+selected+" >"+feedspectrumlist[i]['id']+": "+feedspectrumlist[i]['name']+"</option>";
                }
            }
