...
http://openenergymonitor.org
  ...
  render.js goes through all the dashboardspectrum html elements that specify the dashboardspectrum widgets
...
// Array that holds ID's of feedspectrums of associative key
var feedspectrumids = [];
...
function show_dashboardspectrum()
{
...
function update()
{
  var query = path + "feedspectrum/list.json?userid="+userid;
  $.ajax(
  {
    url : query,
    dataType : 'json',
    ...
    { 
      ...
      for (z in data)
      {
        ...
        feedspectrumids[newstr] = data[z]['id'];
      }
...
function curve_value(feedspectrum,rate)
{
  var val = 0;
  if (feedspectrum) {
    if (!assoc_curve[feedspectrum]) assoc_curve[feedspectrum] = 0;
    assoc_curve[feedspectrum] = assoc_curve[feedspectrum] + ((parseFloat(assoc[feedspectrum]) - assoc_curve[feedspectrum]) * rate);
    val = assoc_curve[feedspectrum] * 1;
  }
  return val;
}
