...
function button_widgetlist()
{
  var widgets = {
    "button":
    {
      ...
      "menu":"Widgets",
      "options":["feedspectrum","value"],
      "optionstype":["feedspectrum","value"],
      "optionsname":[_Tr("feedspectrum"),_Tr("Value")],
      "optionshint":[_Tr("feedspectrum to set, control with caution, make sure device being controlled can operate safely in event of emoncms failure."),_Tr("Starting value")]
    }
  }
...
function button_events()
{
  $('.button').on("click", function(event) {
    var feedspectrumname = $(this).attr("feedspectrum");
    var feedspectrumid = feedspectrumids[feedspectrumname];

    var invalue = $(this).attr("value");
    if (invalue == 0) outval = 1;
    if (invalue == 1) outval = 0;

    feedspectrum.set(feedspectrumid,{'time':parseInt((new Date()).getTime()/1000),'value':outval});
    $(this).attr("value",outval);

    var id = "can-"+$(this).attr("id");
    draw_button(widgetcanvas[id], outval);
    assoc[feedspectrumname] = outval;
  });
}
...
function button_draw()
{
  ...
  {
    var feedspectrum = $(this).attr("feedspectrum");
