...
function jgauge_widgetlist()
{
  var widgets = {
    "jgauge":
    {
      ...
      "menu":"Widgets",
      "options":["feedspectrum", "max", "units"],
      "optionstype":["feedspectrum","value","value"],
      "optionsname":[_Tr("feedspectrum"),_Tr("Max value"),_Tr("Units")],
      "optionshint":[_Tr("feedspectrum"),_Tr("Max value to show"),_Tr("Units to show")]
...
function jgauge_init()
{
  ...
  needle = new Image();
  needle.src = path+'Modules/dashboardspectrum/Views/js/widgets/jgauge/needle2.png';

  // Load the jgauge image
  img = new Image();
  img.src = path+'Modules/dashboardspectrum/Views/js/widgets/jgauge/jgauge.png';
}
...
function jgauge_draw()
{
  $('.jgauge').each(function(index)
  {
    ...
    var val = curve_value(feedspectrum,dialrate);
    // ONLY UPDATE ON CHANGE
    if ((val * 1).toFixed(2) != (assoc[feedspectrum] * 1).toFixed(2) || redraw == 1)
    {
